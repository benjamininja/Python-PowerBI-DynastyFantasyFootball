...
| `fact_roster_transactions` | `season_id + event_type + team_key + asset_id + event_seq` | Event-sourced acquisition ledger = SSOT (ADR-0003 key amended by ADR-0004). **BUILT 2026-06-14** by `02d` (live-loop), 137 `startup_draft` rows (Riddell), grown to 935 (both divisions) by 2026-07-11. `event_type` enum = **startup_draft** (snake draft — corrected from the ADRs' misnamed `startup_auction`; real auctions = FA/re-sign next offseason) / rookie_draft / fa_auction / fa_pickup / resign(+drop) / pick_allocation / trade — dormant v1 / **minor_assignment** / **minor_graduation** (added 2026-07-12, derived from observed per-copy contract transitions across `fact_roster_placement` snapshot history; `event_seq = 1000 + snapshot ordinal` so they rank after startup-draft slots in last-event-wins replay) / **trade** + **trade_away** (2026-07-19) / **claim** + **drop** (2026-07-25, parsed from 04t's `getTransactionDetailsHistory` capture — `transactionCode` is null on a trade leg, `"CLAIM"`/`"DROP"` on FA churn). **All four txn types share ONE chronological `event_seq` (`TXN_SEQ_BASE = 100_000` + position in the datetime-sorted stream)** — trades and FA churn interleave in real time, so per-type seq bases would let 02e's last-event-wins rank an older claim after a newer trade. `trade_away` and `drop` are TERMINAL in 02e. Contract terms are resolved by walking that stream forward with running per-copy `(team_key, asset_id)` + per-conference `(conference, asset_id)` state seeded from the non-txn ledger: a trade moves the copy's existing contract; a **claim** inherits only a contract held *this season in this conference* (conference scoping matters — duplicate-player league, one copy per conference, so the other side's copy must not donate its contract), else falls back to `dim_contract`'s `FA` row ($2,000,000 league minimum). Each pick → Initial contract yr1 (`contract_id`=1st, cap_hit 0.50×), `contract_value` = Fantrax `salary` (as-of capture, via `dim_fantrax_crosswalk` scorer_id→gsis→adp). 18 cols incl. `event_seq`=overall pick#, `event_date`, `scorer_id`/`gsis_id` audit, `draft_round`/`pick_in_round`/`pick_overall`. Source `getDraftResults` via `04w` (**SW-served → HAR can't carry body**; fetch through authed ctx). Idempotent replace-by `(season_id, event_type)` — **except** the four txn types (trade/trade_away/claim/drop), which replace by `event_type` alone since they're rebuilt in full from the 04t capture every run (2026-07-25: needed because `season_id` is now resolved from `dim_season`'s real `season_fantasy_start_date`/`end_date` spans instead of an old `month >= 8` heuristic that mis-filed the June/July 2026 trades under a nonexistent `"2025-2026"` — correcting the season would otherwise strand the old partition as orphan rows). **ETL-only** — not surfaced directly in the PBI model; its value is producing `fact_fantasy_teams` via `02e`'s replay |
| `fact_roster_placement` | `team_id + scorer_id + season + week` | Weekly Fantrax roster-slot snapshot (Active/Reserve/Minors), built by `04v` (Yo-Yo Rule), replace-by-(season, week). Grain is **(team, scorer)**, not scorer alone — duplicate-player league, one copy per conference. **Sole writer is `04v`** — and it is load-bearing: `02e` stamps `fact_fantasy_teams.roster_status` from the latest snapshot, and capmath + the DAX `Active Roster Salary` exempt `roster_status == "Minors"` from the cap charge, so if 04v stops running every Minors-placed player starts charging full salary. Also read by `02d`'s `derive_minor_events` — **inert dead code** (ADR-0011: no Minor contract type exists, so it has never emitted a row). **ETL-only** — no PBI model value, it's an input to 02e |
| `fact_minor_eligibility` | `scorer_id + season + week` | Weekly Yo-Yo Rule eligibility snapshot (Fantrax's own minors-eligibility verdict — GP ≤ 19 career+current, both rostered and FA), built by `04v`. Durable week-over-week history of the eligible population (the raw JSON alone can't answer "when did this player cross 20 GP"). **There is no Minor contract type** (ADR-0011, 2026-07-26 — supersedes ADR-0010, which modelled Minors as a contract with stash-durability rules): Minors is exactly two things, **eligibility** (GP ≤ 19, Fantrax-computed, league-wide, rostered or FA) and **placement** (which squad section a copy occupies — the team's lever, and the sole cap exemption). Minors-eligible players hold ordinary contracts, generally `1st`; placing them in or out of the Minors squad changes nothing about the contract. Confirmed in the data: all 125 Minors-placed copies hold `1st`, zero `Minor` contracts exist. Eligibility permits placement, it doesn't compel it — 357 rostered copies are eligible, only 125 are placed. **ETL-only** — no PBI model value |
