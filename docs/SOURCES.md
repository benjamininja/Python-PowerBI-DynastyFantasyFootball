...
| Source | URL / locator | Purpose | Auth | Feeds (notebook → table) | Cadence |
|---|---|---|---|---|---|
| **Fantrax — getDraftResults / draftPicks.go** | https://www.fantrax.com/fxpa/req?leagueId=v744203wmmvjqzv6 (getDraftResults live-draft; draftPicks.go ownership — capture pending) | Made-pick attribution (live draft) + pick-ownership/trades inventory -> event-sourced ledger | Same as getDraftRanks (Fantrax login + Playwright profile) | `04w_fantrax_draft_results.py` → `fact_roster_transactions` | Between picks (live draft) + on settle |
